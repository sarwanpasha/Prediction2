...
%     Hidden_Users = randi(769,80,1); % generate 5 random values between 1 and 100  (For caltech Dataset)
    Hidden_Users = [769]; % For Caltech36 dataset
    Load_Data_File
end
...
%                         fprintf('skipping\n');
        ...
        Mixing_Matrix
        sum_Of_Matrix=sum(sum(Mixing_Matrix));
        sum_Of_Matrix
        
        sum_Of_Edges=(sum(sum(adjMatrix)));
        sum_Of_Edges
...
%         Column_Sum=sum(Mixing_Matrix);
        ...
        friends_Column_Counter = 1;
        FOF_Column_Counter_1 = 2;
        
        gender_wise_Friends = zeros(Hidden_Count,length(a2_distinct));
        gender_wise_Total_FOF = zeros(Hidden_Count,length(a2_distinct));
        gender_wise_Final_FOF = zeros(Hidden_Count,length(a2_distinct));
        gender_wise_Total_FOF_Percentage = zeros();
        
        gender_wise_FOF = zeros(nodeCount,length(a2_distinct));
        
        for i=1:Hidden_Count
            friends_Row_Counter = 5;
            FOF_Row_Counter_1 = 5;
            neighbor=adjMatrix(Hidden_Users(i),:); % getting neigbours of hidden user
            neighbourCount=sum(neighbor);
            first_Tier_Friends = find(neighbor==1);
            
%             first_str = conv2xlscell(friends_Row_Counter,friends_Column_Counter);
%             first_str_temp = conv2xlscell(friends_Row_Counter-1,friends_Column_Counter);
%             xlswrite('test.xlsx','Friends (y)','Friends',first_str_temp);
%             xlswrite('test.xlsx',first_Tier_Friends','Friends',first_str);
            
            
            FOF_Column_Counter = 1;

            Male_Counter_1 = 0;
            Female_Counter_1 = 0;
            Gender_Counter = zeros(length(a2_distinct),1);
            Second_Tier_Gender_Counter = zeros(length(first_Tier_Friends),length(a2_distinct));
            gender_wise_Second_Tier_Friends = zeros(length(first_Tier_Friends),length(a2_distinct));
            
            for neighbor_record=1: length(first_Tier_Friends)
                
                Male_Counter = 0;
                Female_Counter = 0;
                gender_of_first_Tier_Friends(neighbor_record) = a2(first_Tier_Friends(neighbor_record));
                if(a2(first_Tier_Friends(neighbor_record))~=0)
                    index_count=find(a2(first_Tier_Friends(neighbor_record))==a2_distinct);
                    Gender_Counter(index_count)=Gender_Counter(index_count)+1;
                end
%                 if gender_of_first_Tier_Friends(neighbor_record)==1
%                        Male_Counter_1 = Male_Counter_1 + 1;
% %                        Gender_Counter(a2(first_Tier_Friends(neighbor_record)))=Gender_Counter(a2(first_Tier_Friends(neighbor_record)))+1;
% %                        Gender_Counter(FOF_neighbor_record);
%                 elseif gender_of_first_Tier_Friends(neighbor_record)==2
%                     Female_Counter_1 = Female_Counter_1 + 1;
%                 end
                Male_Counter_1;
                Female_Counter_1;
                FOF=adjMatrix(first_Tier_Friends(neighbor_record),:); % getting neigbours of hidden user
                Second_Tier_Friends=find(FOF==1);
                % Getting Total friend count for each neighbor 
                str_2 = conv2xlscell(neighbor_record,1);
                Friend_Size_For_Second_Tier_Friends(neighbor_record) = length(Second_Tier_Friends); 
%                 xlswrite('test.xlsx',Friend_Size_For_Second_Tier_Friends(neighbor_record),'status3',str_2);
                
                for FOF_neighbor_record=1: length(Second_Tier_Friends) 
                    str_3 = conv2xlscell(FOF_neighbor_record,FOF_Column_Counter);
                    gender_of_Second_Tier_Friends(FOF_neighbor_record) = a2(Second_Tier_Friends(FOF_neighbor_record));
                    if(a2(Second_Tier_Friends(FOF_neighbor_record))~=0)
                        Index_FOF_Counter=find(a2(Second_Tier_Friends(FOF_neighbor_record))==a2_distinct);
                        Second_Tier_Gender_Counter(neighbor_record,Index_FOF_Counter)=Second_Tier_Gender_Counter(neighbor_record,Index_FOF_Counter)+1;
                    end
%                     if gender_of_Second_Tier_Friends(FOF_neighbor_record)==1
%                        Male_Counter = Male_Counter + 1;
% %                        Gender_Counter(FOF_neighbor_record)
%                     elseif gender_of_Second_Tier_Friends(FOF_neighbor_record)==2
%                         Female_Counter = Female_Counter + 1;
%                     end
%                     xlswrite('test.xlsx',gender_of_Second_Tier_Friends(FOF_neighbor_record),'FOF_Gender',str_3);
                end
                
%                 first_str_1 = conv2xlscell(FOF_Row_Counter_1,FOF_Column_Counter_1);
%                 first_str_2 = conv2xlscell(FOF_Row_Counter_1,FOF_Column_Counter_1+1);
%                 xlswrite('test.xlsx',Male_Counter','Friends',first_str_1);
%                 xlswrite('test.xlsx',Female_Counter','Friends',first_str_2);
%                 
%                 first_str_1_temp = conv2xlscell(FOF_Row_Counter_1 - 1,FOF_Column_Counter_1);
%                 first_str_2_temp = conv2xlscell(FOF_Row_Counter_1 - 1,FOF_Column_Counter_1+1);
%                 xlswrite('test.xlsx',Male_Counter','Friends',first_str_1_temp);
%                 xlswrite('test.xlsx',Female_Counter','Friends',first_str_2_temp);
                
                Male_Counter;
                Female_Counter;
                gender_wise_Second_Tier_Friends(neighbor_record,1) = Male_Counter;
                gender_wise_Second_Tier_Friends(neighbor_record,2) = Female_Counter;
                
                FOF_Row_Counter_1 = FOF_Row_Counter_1 + 1;
                FOF_Column_Counter = FOF_Column_Counter + 1;
%                 gender_of_Second_Tier_Friends(neighbor_record) = a2(Second_Tier_Friends(neighbor_record));
            end
            gender_wise_Total_FOF(i,1) = sum(gender_wise_Second_Tier_Friends(:,1));
            gender_wise_Total_FOF(i,2) = sum(gender_wise_Second_Tier_Friends(:,2));
            gender_wise_Total_FOF;
            
            gender_wise_Final_FOF(i,1) = (gender_wise_Total_FOF(i,1) / (gender_wise_Total_FOF(i,1) + gender_wise_Total_FOF(i,2))) * 100;
            gender_wise_Final_FOF(i,2) = (gender_wise_Total_FOF(i,2) / (gender_wise_Total_FOF(i,1) + gender_wise_Total_FOF(i,2))) * 100;

            gender_wise_Friends(i,1) = Male_Counter_1;
            gender_wise_Friends(i,2) = Female_Counter_1;
            gender_wise_Friends;
            
%             gender_wise_FOF()
            
            
            FOF_Column_Counter_1 = FOF_Column_Counter_1 + 4;
            
            prediction_Final=zeros(neighbourCount,length(a2_distinct));
            prediction_Total_Final=zeros(neighbourCount,length(a2_distinct));
                ...
                if neighbor(j)==1
                   ...
                   finding_Max_Vote_class=finding_Max_Vote_class/sum(finding_Max_Vote_class);
                    ...
                    for x=1:length(a2_distinct)
                        ...
                        prediction_Final(friend_count_iterator,x)=Mixing_Matrix(index,x)*finding_Max_Vote_class(x);
...
%             prediction_Final;
            Column_String = conv2xlscell(1,test_Counter);
            test_Counter = test_Counter + 3;
%             xlswrite('test.xlsx',prediction_Final,'',Column_String);
            for final_i=1:neighbourCount
                [m,max_index]= max(prediction_Final(final_i,:));
            ...
            friends_Row_Counter = friends_Row_Counter + 1;
            friends_Column_Counter = friends_Column_Counter + 4;
