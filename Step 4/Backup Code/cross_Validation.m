...
%         fprintf('Second Attribute : %d (%s)\n',attribute2,Name_For_Second_Attribute_Value);
        ...
        for i=1:Hidden_Count
            neighbor=adjMatrix(Hidden_Users(i),:);
            length(neighbor);
            neighbor;
                ...
                if neighbor(j)==1
                   ...
                   for k=1:nodeCount
                       if (Friends_Of_Friend(k)==1 && k~=j)
                           if(a2(k)~=0)
                                finding_Max_Vote_class(find(a2_distinct==a2(k)))=finding_Max_Vote_class(find(a2_distinct==a2(k)))+1;
                           end
%                            if(a2(k)==1)
%                            Male_Friends = Male_Friends+1;
%                            elseif(a2(k)==2)
%                               Female_Friends = Female_Friends+1; 
%                            end
%                             for temp_fof=1: length(a2_distinct)
%                                 if(a2(k)==a2_distinct(temp_fof))
%                                     finding_Max_Vote_class(temp_fof) = finding_Max_Vote_class(temp_fof) + 1;
%                                 end
%                             end
                       end
                   end
%                    max_a2_Vote=max(Male_Friends,Female_Friends);
                   max_a2_Vote=max(finding_Max_Vote_class);
%                    max_a2_Vote_Final = max_a2_Vote / (Male_Friends + Female_Friends);
                   max_a2_Vote_Final = max_a2_Vote / sum(finding_Max_Vote_class);
                   individualMajorFriendCount(i,n) = max_a2_Vote_Final;
...
%         Column_Sum=sum(Mixing_Matrix);
        ...
        mm=zeros(Hidden_Count,length(a2_distinct));
        Mixing_Matrix=bsxfun(@rdivide,Mixing_Matrix,Row_Sum_Formula(:));
        for i=1:Hidden_Count
            
            for k=1:length(a2_distinct)
                mm(i,k)=dot(individualMajorFriendCount(i,:),(Mixing_Matrix(:,k)));
            end
            
%             for j=1:length(a2_distinct)
%                 mm(i,j) = individualMajorFriendCount(i,j) * ((Mixing_Matrix(j,1))/ Row_Sum_Formula(i));
%                 mm(i,j);
%             end
            i;
            j;
            ...
            Predicted_Value(i) = a2_distinct(I); % Predicted classes
            Actual_Value(i) = a2(Hidden_Users(i)); % True classes
            
            if(a2_distinct(I)==a2(Hidden_Users(i)))
